...
class Article extends Component {
  ...
  elapsedTime = () => {
    ...
    if (this.props.article.timestamp) {
      elapsedMilliseconds =
        Date.now() - Date.parse(this.props.article.timestamp);
    } else {
      elapsedMilliseconds =
        Date.now() - Date.parse(this.props.article.createdAt);
    }
    switch (true) {
      case elapsedMilliseconds < 120000:
        return "Just now";
      case elapsedMilliseconds >= 120000 &&
        elapsedMilliseconds < 1000 * 60 * 50:
        return `${Math.round(elapsedMilliseconds / (1000 * 60))} minutes ago`;
      case elapsedMilliseconds >= 1000 * 60 * 50 &&
        elapsedMilliseconds <= 2700000:
        return "1 hour ago";
      ...
      case elapsedMilliseconds > 1814400000:
        return `${Math.round(
          elapsedMilliseconds / (1000 * 60 * 60 * 24 * 7),
        )} weeks ago`;
      default:
  ...
  saveArticle = () => {
    ...
    axios
      .put(`${url}/api/article/${id}/add`, null, config)
      .then(res => {
        console.log("saved");
      })
      .catch(err => {
  ...
  removeArticle = () => {
    ...
    axios
      .put(`${url}/api/article/${id}/del`, null, config)
      .then(res => {
  ...
  reportArticle = clickbait => {
    ...
    axios
      .post(`${urlDs}/${this.props.article.id}`, report)
        ...
        <Card.Content style={{ display: "flex", alignItems: "center" }}>
          ...
          <div>
            ...
            <Grid>
              ...
              {this.props.loggedIn &&
                this.props.articleOptions === "clickbait" && (
                  <ArticleOptions
                    clickbaitModal={this.state.clickbaitModal}
